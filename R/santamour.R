...
#' @param output Either "numeric" for the count of violations or "text" for the species names.
#'
#' @return Either a numeric count of violations of the rule, or the species associated with those violations, depending on output.
#' @export
#'
#' @examples
#' \dontrun{
#' data(TreeTown_Data)
#' santamour(TreeTown_Data$Species, TreeTown_Data$Genus, TreeTown_Data$Family, TreeTown_Data$Region, "numeric")
#' }
santamour <- function(species, genus, family, region, output) {
  ...
  for (reg in unique_regions) {
    ...
    species_perc <- length(unique(species[indices])) / total_count * 100
    ...
    if (species_perc > 10 ||
      ...
      violating_species <- unique(c(violating_species, species[indices]))
    }
  }
